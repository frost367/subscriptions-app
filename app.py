...
.stExpander{background:white!important;border:1.5px solid #ede9fe!important;border-radius:16px!important;margin-bottom:10px!important;}
...
.cl-box{border-radius:12px;padding:8px 4px;text-align:center;color:white;min-height:72px;display:flex;flex-direction:column;justify-content:center;align-items:center;background:linear-gradient(135deg,#3730a3,#6d28d9);}
...
.popup{background:white;border-radius:16px;padding:14px;border:1.5px solid #ede9fe;margin:8px 0;}
...
if st.session_state.sdata is None:
    st.session_state.sdata = load(DATA_FILE) or {"emails":[]}
if st.session_state.pdata is None:
    st.session_state.pdata = load(PROFIT_FILE) or {"products":[],"sales":[]}
...
main_tab1, main_tab2 = st.tabs(["📋 الاشتراكات", "💰 الفائدة"])

with main_tab1:
    st.markdown(f"""
<div class="sub-header" style="background:linear-gradient(135deg,#3730a3,#6d28d9);">
<h2>📋 الاشتراكات <span style="font-size:12px;opacity:.7;">{datetime.today().strftime('%d/%m/%Y')}</span></h2>
<div class="stats-row">
...
<div class="stat-card"><div class="num">{total_paid}</div><div class="lbl">دج</div></div>
    ...
    s1, s2, s3, s4 = st.tabs(["📧 إيميلات", "🔔 تنبيه", "📊 إحصاء", "➕ إضافة"])

    with s1:
        search = st.text_input("", placeholder="🔍 ابحث...", key="srch")
        elist = sdata.get("emails", [])
        if search:
            elist = [e for e in elist if search.lower() in e["email"].lower()]
        if not elist:
        ...
        for ei, em in enumerate(elist):
            ...
            if any(d<=0 for d in dll): icon="🔴"
            elif any(0<d<=2 for d in dll): icon="🟡"
            elif clients: icon="🟢"
            else: icon="⚪"
...
<div class="popup-row"><span class="popup-lbl">💳 الدفع</span><span>{'✅ دفع' if c.get('paid')=='نعم' else '❌ لم يدفع'}</span></div>
                    ...
                    with cc1:
                        if st.button("💾 حفظ", key=f"sn_{ei}_{ci}"):
                            ...
                            save(DATA_FILE, sdata)
                            st.session_state.sdata = sdata
                            st.success("✅")
                    with cc2:
        ...
        for em in sdata.get("emails",[]):
            for c in em.get("clients",[]):
                ...
                if dl<=0:
                    ...
                    st.markdown(f'<div class="alert-w">⚠️ <b>{dl} يوم!</b> {c.get("email","")} | {em["email"]} | {c.get("end","")}</div>', unsafe_allow_html=True)
                    found = True
        if not found:
            st.success("✅ كل الاشتراكات بخير!")

    with s3:
        st.markdown("### 📊 إحصائيات الشهر")
        ...
        if st.button("💾 حفظ الإيميل", key="save_em"):
            if ne.strip():
                if any(e["email"]==ne.strip() for e in sdata["emails"]):
                    ...
                    sdata["emails"].append({
                        "email": ne.strip(), "password": np_,
                        "service": nserv, "start": nstart.strftime("%Y-%m-%d"),
                        "clients": []
                    })
                    save(DATA_FILE, sdata)
                    st.session_state.sdata = sdata
                    st.success(f"✅ تم إضافة {ne}")
                    st.rerun()
            else:
                st.error("أدخل الإيميل!")
...
<div class="stat-card"><div class="num">{m_qty}</div><div class="lbl">مباع</div></div>
...
<div class="stat-card"><div class="num">{m_profit}</div><div class="lbl">دج فائدة</div></div>
    ...
    p1, p2, p3, p4 = st.tabs(["📦 منتج", "🧾 اليوم", "📊 شهري", "➕ جديد"])

    with p1:
        st.markdown("### 📦 قائمة المنتجات")
        prods = pdata.get("products", [])
        if not prods:
            st.info("لا توجد منتجات — أضف من تبويب ➕ جديد")
...
<div style="color:#6b7280;font-size:13px;">💸 التكلفة: {p.get('cost_price',0)} دج · 💰 البيع: {p.get('sell_price',0)} دج</div>
    ...
    with p2:
        ...
        prods = pdata.get("products", [])
        if not prods:
            st.warning("أضف منتجاً أولاً من تبويب ➕ جديد")
        else:
...
<div><div style="font-size:18px;font-weight:900;color:#065f46;">{prv} دج</div><div style="font-size:11px;color:#059669;">الفائدة</div></div>
            ...
            if st.button("✅ تسجيل", key="regsale"):
                pdata["sales"].append({
                    "product": sp, "qty": qty,
                    "sell_price": selp.get("sell_price",0),
                    "cost_price": selp.get("cost_price",0),
                    "date": sd.strftime("%Y-%m-%d")
                })
                save(PROFIT_FILE, pdata)
                st.session_state.pdata = pdata
                st.success(f"✅ {qty}x {sp}")
                st.rerun()
